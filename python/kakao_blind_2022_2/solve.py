"""
Start API
Locations API : 현재 시간에 각 자전거 대여소가 보유한 자전거 수를 반환한다.
Trucks API : 각 트럭의 위치와 싣고 있는 자전거 수를 반환한다.
Simulate API : 현재 시간에 각 트럭이 행할 명령을 담아 서버에 전달한다.
- 만료시간이 된 자전거들을 먼저 반납한다.
- 대여 신청을 봐서 대여 가능한걸 대여해준다.
- 트럭들을 명령에 따라 움직인다.
...
def api_game_result():
    ...
    return [[d['win'], d['lose'], d['taken']]for d in resp]  # 이긴 유저의 아이디, 게임의 승패, 게임에 걸린 시간을 담은 배열
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init-token", type=str, default="7567350842b20f88a735e0eea786b92e")
